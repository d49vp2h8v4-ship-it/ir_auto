...
public class GuiTrainTimetable extends GuiScreen implements StationPickerReceiver, GuiTimetableTemplatePicker.Receiver {
    ...
    public void initGui() {
        ...
        buttonList.add(new GuiButton(18, left + 92, top + 94, 110, 20, "从模板选择"));
    ...
    @Override
    public void applyPickedStationPos(int stopIndex, BlockPos pos) {
        if (pos == null) {
            status = "未选择坐标";
            return;
        }
    ...
    @Override
    public void applyPickedStation(int stopIndex, StationData station) {
        if (station == null) {
            status = "未选择站台";
            return;
        }
    ...
    public void onTemplateLoaded(String templateId) {
        ...
        if (!pendingTemplateId.isEmpty() && !pendingTemplateId.equals(templateId)) {
            return;
        }
        ...
        if (!loaded.hasKey("stops", Constants.NBT.TAG_LIST)) {
            status = "模板无站台数据";
            return;
        }
        NBTTagList templateStops = loaded.getTagList("stops", Constants.NBT.TAG_COMPOUND);
        NBTTagList newStops = new NBTTagList();
        for (int i = 0; i < templateStops.tagCount(); i++) {
            NBTTagCompound s = templateStops.getCompoundTagAt(i);
            if (s == null) {
                continue;
            }
            NBTTagCompound stop = s.copy();
            if (stop.hasKey("trainNumber")) {
                stop.removeTag("trainNumber");
            }
            newStops.appendTag(stop);
        }
