...
public class GuiRailwayMapStationPicker extends GuiScreen {
    ...
    private final StationPickerReceiver receiver;
    ...
    public GuiRailwayMapStationPicker(GuiScreen parent, StationPickerReceiver receiver, int stopIndex, BlockPos selected) {
        this.parent = parent;
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (mouseButton == 0) {
            ...
            if (picked != null) {
                ...
                if (receiver != null) {
                    receiver.applyPickedStation(stopIndex, picked);
                }
                mc.displayGuiScreen(parent);
                return;
            }
    ...
    private StationData pickStationByMouse(int mouseX, int mouseY) {
        ...
        for (StationData station : RailMapClientCache.getStations()) {
            if (station.x < minX || station.x > maxX || station.z < minZ || station.z > maxZ) continue;
            int x = (int) (left + 4 + (station.x - minX) * scaleX);
            int y = (int) (top + 4 + (station.z - minZ) * scaleZ);
            int dx = mouseX - x;
            int dy = mouseY - y;
