...
public class ClientTurnoutPreview {
    @SubscribeEvent
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        ItemStack held = main.getItem() == ModItems.TURNOUT_MACHINE ? main : (off.getItem() == ModItems.TURNOUT_MACHINE ? off : ItemStack.EMPTY);
        if (held.isEmpty()) {
            return;
        }
        NBTTagCompound tag = held.getTagCompound();
        if (tag == null || !tag.hasKey(ItemTurnoutMachine.NBT_RAIL)) {
            return;
        }
        BlockPos rail = BlockPos.fromLong(tag.getLong(ItemTurnoutMachine.NBT_RAIL));
