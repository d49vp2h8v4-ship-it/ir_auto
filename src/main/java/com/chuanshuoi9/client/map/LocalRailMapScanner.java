...
public class LocalRailMapScanner {
        ...
        mc.getIntegratedServer().addScheduledTask(() -> {
            try {
                ...
                mc.addScheduledTask(() -> {
                    ...
                    if (stations != null) RailMapClientCache.updateStations(stations);
                });
            } finally {
                IN_FLIGHT.set(false);
            }
    ...
    private static ScanResult scanLoadedAround(WorldServer world, BlockPos centerPos) {
        ...
        for (int chunkX = minChunkX; chunkX <= maxChunkX; chunkX++) {
            for (int chunkZ = minChunkZ; chunkZ <= maxChunkZ; chunkZ++) {
                ...
                for (TileEntity tileEntity : chunk.getTileEntityMap().values()) {
                    ...
                    if (tileEntity instanceof ITrack) {
                        Long k = IrTrainReflection.getLogicalRailKey(tileEntity);
                        BlockPos pos = k == null ? tileEntity.getPos() : BlockPos.fromLong(k);
                        positions.add(pos.toImmutable());
                        if (k != null) entryKeys.add(k);
                    } else if (isImmersiveRailTile(tileEntity) || isImmersiveRailBlock(tileEntity)) {
        ...
        return new ScanResult(positions, entryKeys, signalPaths, seenSignals, scannedChunks, minX, maxX, minZ, maxZ);
    }

    private static class ScanResult {
        private final Set<BlockPos> railPositions;
        private final Set<Long> entryKeys;
        private final Map<Long, Set<Long>> signalPaths;
        private final Set<Long> seenSignals;
        private final Set<Long> scannedChunks;
        private final int minX, maxX, minZ, maxZ;

        private ScanResult(Set<BlockPos> railPositions, Set<Long> entryKeys, Map<Long, Set<Long>> signalPaths,
                          Set<Long> seenSignals, Set<Long> scannedChunks,
