...
import com.chuanshuoi9.signal.Signal3Controller;
...
public class TrainAutoPilotData {
    ...
    public static void tickAutoDrive(Entity train) {
        ...
        // 三状态信号机控制：列车进入区间即受控，RED 紧急制动
        Signal3Controller.SpeedLimit s3 = Signal3Controller.getSpeedLimit(train.world, train);
        if (s3 != null) {
            if (s3.emergencyBrake) {
                // RED → 紧急制动，直接停
                root.setInteger(OVERSPEED_COUNT, 0);
                IrTrainReflection.applyThrottleAndBrake(train, 0, 1.0f);
                train.getEntityData().setTag(ROOT, root);
                return;
            }
            signalSpeedLimit = s3.speedKmh;
        }
        ...
        if (signalCmd != null) {
            ...
            signalSpeedLimit = signalSpeedLimit == null ? signalCmd.speedLimitKmh : Math.min(signalSpeedLimit, signalCmd.speedLimitKmh);
        }
